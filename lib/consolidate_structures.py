...
def consolidate_and_convert_structures(df, prefix='', smiles='Smiles', inchi=0):
    ...
    print(' ==== Consolidating structures from SMILES and/or InChI ====')
    ...
    if inchi == 0:
        ...
        df[smiles] = df[smiles].map(lambda x:x.lstrip('"').rstrip('"'))
        df[inchi] = df[inchi].map(lambda x:x.lstrip('"').rstrip('"'))
        
        #Resolve cropped inchi
        df[inchi] = df[inchi].replace({'1S/':'InChI=1S/'}, regex=True)
        df[inchi] = df[inchi].replace({'InChI=InChI=1S/':'InChI=1S/'}, regex=True)
